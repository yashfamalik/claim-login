(function (global) {
  ...
  // SECURITY: Restrict usage to specific files only
  function validateUsage() {
    const allowedFiles = [
      'WidgetSettings.jsx',
      'CustomizationModel.jsx'
    ];
    
    const stack = new Error().stack;
    
    // Strict validation - only allow direct usage from specific files
    const isDirectlyAllowed = allowedFiles.some(file => 
      stack && stack.includes(file)
    );
    
    // Block ExternalTemplate.jsx and any other files
    const isBlocked = stack && (
      stack.includes('ExternalClaimLogin.jsx') ||
      stack.includes('ExternalTemplate.jsx') ||
      stack.includes('TemplateDemo.jsx') ||
      stack.includes('ClaimLogin') ||
      stack.includes('Login') ||
      stack.includes('External') ||
      stack.includes('Preview') ||
      stack.includes('Demo') ||
      stack.includes('claim-login') ||
      stack.includes('FinalLoginForm')
    );
    
    if (isBlocked) {
      console.error('Template.js: BLOCKED - This component cannot be used in login forms or external templates');
      return false;
    }
    
    if (!isDirectlyAllowed) {
      console.error('Template.js: Unauthorized usage detected. This file can ONLY be used in WidgetSettings.jsx and CustomizationModel.jsx');
      return false;
    }
  ...
  function Template(containerId, options = {}) {
    ...
    if (!validateUsage()) {
      throw new Error('Template.js: Unauthorized usage. This component is restricted to specific files only.');
    }
    
    // Additional runtime check for container IDs that should be blocked
    const blockedContainerIds = [
      'claim-login-container',
      'preview_login_form',
      'login-container',
      'external-login',
      'claim-form'
    ...
    if (blockedContainerIds.some(id => containerId && containerId.includes(id))) {
      throw new Error('Template.js: Cannot be used with login or claim form containers.');
    }
  ...
  if (typeof module !== "undefined" && module.exports) {
    ...
    // Add usage validation to global Template
    const OriginalTemplate = Template;
    global.Template = function(containerId, options = {}) {
      if (!validateUsage()) {
        throw new Error('Template.js: Unauthorized usage. This component is restricted to specific files only.');
      }
      return new OriginalTemplate(containerId, options);
    };
    
    // Copy all static methods and properties
    Object.setPrototypeOf(global.Template, OriginalTemplate);
    Object.assign(global.Template, OriginalTemplate);
  ...
  // NO AUTO-INITIALIZATION - Manual initialization only from authorized files

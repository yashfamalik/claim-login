(function (global) {
  ...
  function Template1(containerId, options = {}) {
    ...
    function iconHTML() {
      ...
      const logo = CUSTOM_LOGO_MAP[logoKey];
      if (logo) {
        return `<div class="T1_icon-custom">${logo}</div>`;
      }
      return "";
    }
    ...
    function inlinePointsHTML() {
      ...
      return `<div class="T1_inline-points">${state.inlinePoints
        .map(
          (p) => `<div class="T1_inline-point" style="color: ${darkerTextColor}">${svg(p.icon)}<span>${p.text}</span></div>`
        )
        .join("")}</div>`;
    }
    ...
    function renderT1() {
      state.container.innerHTML = `<div class="T1_shipping-protection">${logoHTML()}<div class="T1_icon-container">${iconHTML()}</div><div class="T1_content"><p class="T1_title">${state.title} ($${state.protectionPrice.toFixed(
        2
      )})</p><p class="T1_description">${state.description}</p></div>${state.selectionMode === "checkbox" ? checkboxHTML() : toggleHTML()}</div>`;
    }
    ...
    function renderT2() {
      ...
      state.container.innerHTML = `<div class="T1_template2-container"><div class="T1_main-widget">${logoHTML()}<div class="T1_icon-container">${iconHTML()}</div><div class="T1_content"><p class="T1_title">${state.title} ($${state.protectionPrice.toFixed(
        2
      )})</p><p class="T1_description">${state.description}</p></div><div class="T1_controls-section">${state.selectionMode === "toggle" ? toggleHTML() : ""}${
      ...
      }</div></div>${inlinePointsHTML()}${
        showDetail
          ? `<div class="T1_protection-added"><div class="T1_icon-container-message">${svg("check")}</div><span>${
              state.protectionAddedText
            }</span><span> (+$${state.protectionPrice.toFixed(2)})</span></div><div class="T1_protection-message"><div class="T1_icon-container-message">${svg(
              "shield"
    ...
    function renderT3() {
      ...
      state.container.innerHTML = `<div class="T1_template3-container"><div class="T1_main-widget" style="position:relative;">${logoHTML()}<div class="T1_icon-container">${iconHTML()}</div><div class="T1_content"><p class="T1_title">${state.title} ($${state.protectionPrice.toFixed(
        2
      )})</p><p class="T1_description">${state.description}</p></div><div class="T1_badge ${
        showBadge ? "T1_visible" : ""
      }">${state.badgeText}</div><div class="T1_controls-section">${state.selectionMode === "toggle" ? toggleHTML() : ""}${
        state.selectionMode === "checkbox" ? checkboxHTML() : ""
      }${
        state.selectionMode === "button" ? `<a href="${state.ctaButtonHref}" class="T1_protection-button">${state.buttonBlock.buttonLabel}</a>` : ""
      }</div></div><div class="T1_confirmation-message ${state.isEnabled ? "T1_visible" : ""}">${state.confirmationMessage}</div></div>`;
    }
    ...
    function updateContentSettings(settings = {}) {
      ...
      if (settings.protectionMessageSubtext !== undefined) state.protectionMessage.subtext = settings.protectionMessageSubtext;
      render();
      bindEvents();
    }
    ...
    function updateCustomLogos(logoUpdates = {}) {
      Object.keys(logoUpdates).forEach((logoKey) => {
        if (CUSTOM_LOGO_MAP[logoKey]) {
          CUSTOM_LOGO_MAP[logoKey] = { ...CUSTOM_LOGO_MAP[logoKey], ...logoUpdates[logoKey] };
        }
      });
      render();
      bindEvents();

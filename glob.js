...
// Auto-initialize Template when script loads
(function() {
  function autoInitialize() {
    // Look for existing container or create one
    let container = document.getElementById('template-widget-container');
    
    if (!container) {
      // Create container and inject it into the page
      container = document.createElement('div');
      container.id = 'template-widget-container';
      container.style.cssText = 'margin: 20px 0; max-width: 600px;';
    ...
    // Initialize Template with default settings
    if (typeof Template === 'function') {
      const templateInstance = new Template('template-widget-container', {
        activeTemplate: "Template1",
        isEnabled: true,
        ...
        onToggle: function(enabled) {
          ...
          window.dispatchEvent(new CustomEvent('templateProtectionToggle', {
            detail: { enabled: enabled, price: this.protectionPrice || 12.11 }
          }));
        }
      });
      
      // Make instance globally available
      window.TemplateWidget = templateInstance;
      
      // Dispatch ready event
      window.dispatchEvent(new CustomEvent('templateWidgetReady', {
        detail: { instance: templateInstance }
      }));
    }
  }

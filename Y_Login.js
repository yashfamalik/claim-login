...
window.ClaimLogin = {
  ...
  function render(el) {
    ...
    el.innerHTML = `
      <div style="background:${c.cardBg};padding:16px;border-radius:12px;">
        <h3 style="margin:0 0 12px;color:${c.headingText};">Login</h3>
        <button style="
  ...
  function mount(el) {
    render(el);
    proxy.subscribe(() => render(el)); // <— critical for reactivity
  }

  // Global expose for console use
  global.ClaimLogin = { mount, proxy };
  global.ClaimFormProxy = proxy;
})(window);
